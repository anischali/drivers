				...
				struct hid_time_state *time_state)
{
	...
	for (i = 0; i < TIME_RTC_CHANNEL_MAX; ++i) {
		dev_info(&pdev->dev, "report ID: 0x%02x\n", report_id & 0xff);
...
static int hid_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = wait_for_completion_killable_timeout(
			&time_state->comp_last_time,  msecs_to_jiffies(1000));
...
hid_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (tm->tm_year > 70)
		raw_time[1] = (tm->tm_year - 100) & 0xFF;
	else
		raw_time[1] = (tm->tm_year) & 0xFF;	
	
	raw_time[2] = (tm->tm_mon + 1) & 0xFF;
	raw_time[3] = (tm->tm_mday) & 0xFF;

	raw_time[4] = (tm->tm_hour) & 0xFF;
	raw_time[5] = (tm->tm_min) & 0xFF;
	raw_time[6] = (tm->tm_sec) & 0xFF;
...
int hid_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	raw_time[0] = report_id & 0xFF;
	if (time->tm_year > 70)
		raw_time[1] = (time->tm_year - 100) & 0xFF;
	else
		raw_time[1] = (time->tm_year) & 0xFF;	
	
	raw_time[2] = (time->tm_mon + 1) & 0xFF;
	raw_time[3] = (time->tm_mday) & 0xFF;

	raw_time[4] = (time->tm_hour) & 0xFF;
	raw_time[5] = (time->tm_min) & 0xFF;
	raw_time[6] = (time->tm_sec) & 0xFF;
	raw_time[7] = 1;
...
static int hid_time_probe(struct platform_device *pdev)
{
	...
	time_state->rtc = devm_rtc_device_register(&pdev->dev,
					"hid-sensor-time", drv_data->hid_time_rtc_ops,
					THIS_MODULE);
